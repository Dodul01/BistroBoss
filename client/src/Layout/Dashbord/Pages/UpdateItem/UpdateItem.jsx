...
const UpdateItem = () => {
    ...
    return (
        <div>
            <SectionHeading heading={'update item'} subHeading={"let's update"} />
            <div className="flex items-center justify-center w-full">
                <form onSubmit={handleUpdateItem} className="bg-[#E8E8E8] p-4 rounded max-w-screen-lg w-full mb-4">
                    <div className="my-2">
                        <label className="block mb-2 text-lg font-medium text-gray-900">Recipe name*</label>
                        <input defaultValue={name} className="bg-gray-50 border border-gray-300 text-gray-900 text-lg rounded-lg block w-full p-3" name="recipeName" placeholder="Recipe name" required />
                    </div>
                    <div className="flex gap-3 w-full my-2">
                        <div className="flex-1">
                            <label className="block mb-2 text-lg font-medium text-gray-900">Category*</label>
                            <select defaultValue={category} className="bg-gray-50 border border-gray-300 text-gray-900 text-lg rounded-lg block w-full p-3" name="category" required>
                                <option className="text-gray-900" value="null" disabled>Select category</option>
                        ...
                        <div className="flex-1">
                            ...
                            <input defaultValue={price} className="bg-gray-50 border border-gray-300 text-gray-900 text-lg rounded-lg block w-full p-3" name="price" placeholder="Price" required />
                        </div>
                    </div>
                    <div>
                        <label className="block mb-2 text-lg font-medium text-gray-900">Recipe Details*</label>
                        <textarea defaultValue={recipe} rows="4" className="block p-3 w-full text-lg text-gray-900 bg-gray-50 rounded-lg border border-gray-300" name="details" placeholder="Recipe Details" required></textarea>
                    </div>
                    <div>
                    ...
                    <button className="flex items-center gap-2 p-3 mt-4 rounded text-lg font-semibold bg-gradient-to-r from-[#835D23] to-[#B58130] text-white">Update Item <FaUtensils /></button>
                </form>
            </div>
        </div>
    )
